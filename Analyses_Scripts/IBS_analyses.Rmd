...
#Perform hierarchical clustering 1st group
```{r}
mat1=relat %>% subset(ID1 %in% group1$ID & ID2 %in% group1$ID)
mat1=mat1[,c("ID1","ID2","RELATEDNESS_AJK")]
mat1=dcast(mat1,ID1~ID2,drop=T)
mat1=data.frame(mat1)
rownames(mat1)=mat1[,1]
mat1=mat1[,-1]
#Convert upper triangle matrix to lower triangle
mat1=as.matrix(mat1)
mat1[lower.tri(mat1)]=t(mat1)[lower.tri(mat1)]
colnames(mat1)=rownames(mat1)

#Replace NA values (don't know why they occur for technical replicates)
for(i in 1:nrow(mat1)){
  for(j in 1:ncol(mat1)){
    if(is.na(mat1[i,j])){
      rowname=rownames(mat1)[i]
      colname=colnames(mat1)[j]
      mat1[i,j]=(relat %>% subset(ID1==rowname & ID2==colname | ID2==rowname & ID1==colname))$RELATEDNESS_AJK
    }
  }
}

#Convert simmilarity to dissimilarity matrix 
mat1=1-mat1
...
par(mar=c(4,2.1,0.5,0.5))
hc1 %>% dendextend::set("labels",clones_labels) %>% dendextend::set("labels_cex",0.2) %>% dendextend::hang.dendrogram() %>% dendextend::assign_values_to_leaves_edgePar(value=clones_color,edgePar="col") %>% dendextend::set("branches_lwd",0.5)  %>% plot(ylim=c(-0.02,1.1),yaxt="n")
...
#Second group
```{r}
mat2=relat %>% subset(ID1 %in% group2$ID & ID2 %in% group2$ID)
mat2=mat2[,c("ID1","ID2","RELATEDNESS_AJK")]
mat2=dcast(mat2,ID1~ID2,drop=T)
...
#Only show labels of samples with high similarity 
mat_clones=mat2 %>% replace(.,col(.)==row(.),NA)
min=apply(mat_clones,2,function(x)min(x,na.rm=T))
clones_labels=labels(hc2)
min=min[order(match(names(min),clones_labels))]
names(min)[min>0.3]=""
clones_labels=names(min)
...
library(dendextend)
png(paste0(outpath,"Group2_relatedness_dendogram.png"),width=2000,height=1000,res=400)
par(mar=c(4,2.1,0.5,0.5))
hc2 %>% dendextend::set("labels",clones_labels) %>% dendextend::set("labels_cex",0.2) %>% dendextend::hang.dendrogram() %>% dendextend::assign_values_to_leaves_edgePar(value=clones_color,edgePar="col") %>% dendextend::set("branches_lwd",0.5)  %>% plot(ylim=c(-0.02,1.1),yaxt="n")
# hc1 %>% dendextend::set("labels",clones_labels) %>% dendextend::set("labels_cex",0.2) %>% dendextend::hang.dendrogram() %>% dendextend::assign_values_to_leaves_edgePar(value=clones_color,edgePar="col") %>% dendextend::set("branches_lwd",0.5)  %>% plot(ylim=c(0,1),yaxt="n")
abline(h=replicate_threshold,col="red",lty=3)
axis(2,cex.axis=0.5)
dev.off()
...
#Make single graph with both method plotted together 
```{r}
