...
```{r}
path='/nvme/disk0/lecellier_data/WGS_GBR_data/Raw_data_processing/BAM_statistics/'



#Trimmed statistics -> problem with disk 
df_trim=list.files("/nvme/disk0/lecellier_data/WGS_GBR_data/Trimmed_files/","*sum.txt",full.names = T) %>% lapply(function(x)read.table(x,sep=":",col.names=c("Var","Value")) %>% mutate(sample.name=sub("_sum.*", "",basename(x)))) %>% bind_rows %>% mutate(Value=as.numeric(str_remove(str_replace(Value,",",".")," ")))
...
map_chr=cov_stat_uniq %>% group_by(individualID) %>% dplyr::summarize(numreads_uniq=sum(numreads)) %>% merge(cov_stat %>% subset(Ref=="Amillepora") %>% group_by(individualID) %>% dplyr::summarize(numreads_notuniq=sum(numreads)) %>% dplyr::select(individualID,numreads_notuniq),by="individualID") %>% merge(df_stat %>% subset(Ref=="Amillepora") %>% group_by(individualID) %>% dplyr::summarize(mapped=sum(mapped),unmapped=sum(unmapped)) %>% dplyr::select(individualID,mapped,unmapped),by="individualID")%>% ggplot(aes(x=individualID,y=100*numreads_uniq/(mapped+unmapped))) + geom_col(colour='black',fill='white',position='dodge2') + theme_classic() + labs(x='Sample',y='% Mapped reads',fill='Ref genome') + theme(axis.text.x = element_text(angle=45,vjust = 1, hjust = 1)) + coord_cartesian(ylim=c(80,100))
```


#Save plots 
```{r}
path_save="/home/hdenis/Figures/"
