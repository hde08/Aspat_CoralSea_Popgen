...
#Create file IDs
>/home/hugo/PhD/Genomics/Raw_data_processing/aln_ids.txt
for FILE in ${FILES[@]}; do
	BASE=$(basename $FILE)
	BASE=${BASE%%_U*} 
	echo ${BASE} >> /home/hugo/PhD/Genomics/Raw_data_processing/aln_ids.txt
done 
...
#5.2 Mark and remove duplicates (parallel mode)

start=`date +%s`
cat /home/hugo/PhD/Genomics/Raw_data_processing/aln_ids.txt | parallel --jobs 25 "/home/gael/Programmes/gatk-4.3.0.0/gatk MarkDuplicates INPUT="${INDIR}{}_UNDEDUP_RG.bam" OUTPUT="${INDIR}{}_MARKED_DUP.bam" METRICS_FILE="${OUTDIR}{}_marked_dup_metrics.txt" REMOVE_DUPLICATES=true TMP_DIR="${INDIR}" VALIDATION_STRINGENCY=LENIENT"
end=`date +%s`
echo Execution time was `expr $(( ($end - $start) / 60))` minutes.
...
#Add read group : Single file mode 
