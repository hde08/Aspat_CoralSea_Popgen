...
meta_site=openxlsx::read.xlsx(paste0(path,"Summary_PhD_sites.xlsx"),sheet = "Sheet 1")
...
cov.scores=cov.scores %>% merge(meta_colony %>% dplyr::select(Geographic_cluster,wgsID_short),by="wgsID_short")
...
migration_rate=read.table(paste0(path,"Bootstrap_parameters_allpairs_long.txt"),header=T) 

migration_rate$Pop_short=sub("_20mis","",migration_rate$Pop)
migration_rate=migration_rate %>% subset(Real_parameter %in% c("M12","M21")) %>% merge(arrow_coordinates,by.x="Pop_short",by.y="Pop") %>% cbind(curvature=c(0.3,0.5,0.2,-0.3,0.5,0.4,-0.2,0.4,-0.3,-0.3,0.2,0.2),arrow_dir=c("backward","forward","backward","forward","backward","forward","backward","forward","backward","forward","backward","forward"),linetype=c("dashed","solid","dashed","solid","solid","solid","dashed","solid","solid","solid","solid","dashed"),arrowsize=c(0.01,0.03,0.01,0.03,0.03,0.03,0.01,0.03,0.03,0.03,0.03,0.01)) 


site_map_pie_arrow=site_map_pie

#site_map_pie_arrow=ggplot()
...
for (i in seq_len(nrow(migration_rate))){
  ...
  arrowsize=migration_rate$arrowsize[i]
  
  print(curv)
  
  if (row$arrow_dir == "forward") {
    site_map_pie_arrow <- site_map_pie_arrow + annotate("curve",size=row$median*coeff,
      x = row$x, y = row$y,
      xend = row$xend, yend = row$yend,
      curvature = curv,
      linetype=linetype,
      arrow = arrow(length = unit(arrowsize, "npc"),type="closed"),
      color = "#092347"
    )
...
migration_rate$median_val=seq(1,15,length.out=12)
library(scales)
#Create dummmy legend 
dummy=ggplot(data=migration_rate) + geom_curve(aes(x=x,y=y,yend=yend,xend=xend,size=coeff*median_val)) +labs(size = "Gene flow") +
  scale_size_continuous(
    range=c(0.3,4.5),
    breaks=c(0.3,1.5,3,4.5),
    labels=c("1 individual","5 individuals","10 individuals","15 individuals"),
    guide = guide_legend(
      title.position = "top",       
      title.hjust = 0.5,            
      label.position = "right",    
      label.theme = element_text(size = 18,color="#092347"),  
      direction = "vertical",    
      override.aes = list(color = "#092347"))) + theme(legend.background = element_rect(fill = NA, color = NA),legend.key = element_rect(fill = NA)) + theme(legend.title = element_text(size = 22, color = "#092347"),legend.background = element_rect(
      fill = "transparent",        # background color of the box
      color = "#092347",       # border color
...
site_map_pie_arrow=site_map_pie_arrow + draw_plot(size_legend,x=164,y=-16.3,width=1,height=1)

#ggsave(site_map_pie_arrow,filename=paste0(figpath,"Map_last_version.png"),width=22,height=10,dpi=500)
```
...
ggsave(total_plot,filename=paste0(figpath,"Figure_1_v4.png"),width=22,height=10,dpi=500)
