...
#SBATCH --array=1-50%10        	# job array
...
#mkdir /nvme/disk0/lecellier_data/WGS_GBR_data/Raw_data_processing/BAM_statistics 
...
then
  ...
  /home/hdenis/Programs/gatk-4.5.0.0/gatk AddOrReplaceReadGroups INPUT="${INDIR}${BASE}_UNDEDUP.bam" OUTPUT="${INDIR}${BASE}_UNDEDUP_RG.bam" RGID=$(echo ${BASE} | cut -d"_" -f1 | sed "s/RRAP-.*-202.*-A/A/") RGLB=$(echo ${BASE} | cut -d"-" -f1,2,3) RGPL=ILLUMINA RGPU=$(echo ${BASE} | cut -d"_" -f2) RGSM=$(echo ${BASE} | cut -d"_" -f1 | sed "s/RRAP-.*-202.*-A/A/")
