...
#start=`date +%s`
#angsd -bam $INDIR/aspat_bam_clean.filelist.txt -out "${OUTDIR}GBR_aspatsamples_filt_05mis_${CONTIG}" -ref $REF_3 -r $CONTIG: -nThreads 5 $TODO $FILTERS -minMaf 0.05
#end=`date +%s`
#echo Execution time was `expr $(( ($end - $start) / 60))` minutes. -> Not run so far

##Run a third time on aspat samples and output only sites with MAF >0.05 
#
#start=`date +%s`
#angsd -bam $INDIR/aspat_bam_clean.filelist.txt -out "${OUTDIR}GBR_aspatsamples_filt_05mis_MAF0.05_${CONTIG}" -ref $REF_3 -r $CONTIG: -nThreads 5 $TODO $FILTERS -minMaf 0.05
#end=`date +%s`
#echo Execution time was `expr $(( ($end - $start) / 60))` minutes.
...
#rm /nvme/disk0/lecellier_data/WGS_GBR_data/ANGSD_files/GBR_aspatsamples_filt_05mis_uniq_scaffold_*.beagle.gz
