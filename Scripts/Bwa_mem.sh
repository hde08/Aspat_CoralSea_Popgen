...
#Alignment on A.millepora reference genome 
start=`date +%s`
for REF in ${REFS[@]}; do
	REF_NAME=${REF%%_ncbi*}
	...
	#### 4.2. Map to reference genome 
		bwa mem -t 20 ${REF} -o "${OUTDIR}${BASE}_pe_aln_${REF_NAME}.sam" "${INDIR}${BASE}_R1_paired.fastq.gz" "${INDIR}${BASE}_R2_paired.fastq.gz"

	#### 4.3. Create sorted bam  
		samtools view -b "${OUTDIR}${BASE}_pe_aln_${REF_NAME}.sam" > "${OUTDIR}${BASE}_pe_aln_${REF_NAME}_UNSORTED.bam"
		samtools sort "${OUTDIR}${BASE}_pe_aln_${REF_NAME}_UNSORTED.bam" -O BAM -o "${OUTDIR}${BASE}_pe_aln_${REF_NAME}_UNDEDUP.bam"
